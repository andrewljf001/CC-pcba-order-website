...
### ② PCB + SMT Assembly
- 徽章：`FROM $200`
- 描述：We fabricate your PCB and assemble components. Customer-supplied or sourced by us. Standard pricing up to 10 pcs.
...
| 档位 | 价格 | 条件 |
|------|------|------|
| 一档 | $50 | FR4，钻孔 ≥ 0.3mm，不含阻抗控制，层数 ≤ 4L，≤ 20×20cm，≤ 10片 |
| 二档 | $75 | FR4，钻孔 ≥ 0.3mm，含阻抗控制，层数 ≤ 4L，≤ 20×20cm，≤ 10片 |
| 三档 | $100 | FR4，钻孔 0.2/0.25mm，含阻抗控制，层数 ≤ 4L，≤ 20×20cm，≤ 10片 |

层数选项：**1L / 2L / 4L / 6L / 8L**（只有双数，6L/8L 触发人工报价）

触发人工报价（任一满足）：层数 > 4L、材质非 FR4、钻孔 < 0.2mm、尺寸 > 20×20cm、数量 > 10片

### 4.2 SMT + DIP 标准报价（全包价，其余全部人工报价）

| 条件 | 价格 |
|------|------|
| ≤ 10片，SMT点数 ≤ 200，复杂IC < 10个，单面，元件客供 | **$200** |
| ≤ 10片，SMT点数 ≤ 200，复杂IC < 10个，双面，元件客供 | **$400** |
| 插件 ≤ 100脚 | 含在全包价内，不另收费 |
| 其余 | 人工报价 |

> 复杂 IC 定义：BGA / QFN / LGA / BTC 等不可视焊点封装
...
| 钻孔最小尺寸 | ≥ 0.3mm / 0.25mm / 0.2mm / 0.15mm |
| 阻抗控制 | No / Yes |
| 尺寸 | 宽 × 高（mm） |
| 数量 | 数字输入 |
...
| 数量 | 数字输入 |
| SMT 贴片点数 | 数字输入，无则填 0 |
| DIP 插件脚数 | 数字输入，无则填 0，≤ 100 不另收费 |
| 复杂 IC 数量 | 数字输入（BGA/QFN/LGA/BTC），≥ 10 触发人工 |
| 贴片面 | 单面 / 双面 |
