...
## 人工测试清单（待执行）
在浏览器完整走一遍真实下单，验证今天修的 bug 在真实环境（带Turnstile）也正常：
1. 打开 pcbaforge.com 首页 → 4张图正常、报价计算器能用
2. 首页选服务填参数 → 价格显示正确
3. 跳转 quote.html → 参数带过去、价格与首页一致
4. 填信息+过Turnstile+提交 → 出现订单号
5. 查收邮件 → 收到询价确认邮件
6. 后台登录→找订单→改报价 → 改报价能成功（重点验证今天修的bug）
7. track.html 输订单号+邮箱查询 → 状态变quoted、显示报价
